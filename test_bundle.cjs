...
function findDiffStart(a, b2, pos) {
  for (let i = 0; ; i++) {
    ...
    if (childA.isText && childA.text != childB.text) {
      let tA = childA.text, tB = childB.text, j3 = 0;
      for (; tA[j3] == tB[j3]; j3++)
        pos++;
      if (j3 && j3 < tA.length && j3 < tB.length && surrogateHigh(tA.charCodeAt(j3 - 1)) && surrogateLow(tA.charCodeAt(j3)))
        pos--;
      return pos;
    }
  ...
  compatible(other) {
    for (let i = 0; i < this.next.length; i++)
      for (let j3 = 0; j3 < other.next.length; j3++)
        if (this.next[i].type == other.next[j3].type)
          return true;
    return false;
  }
  /**
    ...
    if (toRemove && toRemove.length) {
      ...
      for (let i = 0; i < toRemove.length; i++) {
        ...
        for (let j3 = 0; j3 < matched.length; j3++) {
          let m2 = matched[j3];
          if (m2.step == step - 1 && style2.eq(matched[j3].style))
            found2 = m2;
        }
        if (found2) {
          found2.to = end;
    ...
    if (!allowed) {
      ...
      for (let j3 = 0; j3 < child.marks.length; j3++)
        if (!parentType.allowsMarkType(child.marks[j3].type))
          tr.step(new RemoveMarkStep(cur, end, child.marks[j3]));
...
function replaceRange(tr, from, to, slice) {
  ...
  for (let j3 = slice.openStart; j3 >= 0; j3--) {
    let openDepth = (j3 + preferredDepth + 1) % (slice.openStart + 1);
          ...
          if (tr && newState.filterTransaction(tr, i)) {
            ...
            if (!seen) {
              ...
              for (let j3 = 0; j3 < this.config.plugins.length; j3++)
                seen.push(j3 < i ? { state: newState, n: trs.length } : { state: this, n: 0 });
            }
            trs.push(tr);
  ...
  parseRange(from, to, base2 = 0) {
    ...
    for (let offset = base2, i = 0; ; i++) {
      ...
      if (fromOffset > -1 && (end > to || i == this.children.length - 1)) {
        to = end;
        for (let j3 = i + 1; j3 < this.children.length; j3++) {
...
function readHTML(html) {
  ...
  for (let i = 0; i < doc3.styleSheets.length; i++) {
    ...
    for (let j3 = 0; j3 < style2.rules.length; j3++) {
      let rule = style2.rules[j3];
...
function mapChildren(oldChildren, newLocal, mapping, node, offset, oldOffset, options) {
  ...
  if (mustRebuild) {
    ...
    for (let i = 0, j3 = 0; i < built.children.length; i += 3) {
      let from = built.children[i];
      while (j3 < children.length && children[j3] < from)
        j3 += 3;
      children.splice(j3, 0, built.children[i], built.children[i + 1], built.children[i + 2]);
    }
  }
...
function removeOverlap(spans) {
  ...
  for (let i = 0; i < working.length - 1; i++) {
    ...
    if (span.from != span.to)
      for (let j3 = i + 1; j3 < working.length; j3++) {
        let next = working[j3];
...
var insertDefaultBlock = (options = {}) => ({ tr, dispatch, editor: editor2 }) => {
...
var setContent = (content, { errorOnInvalidContent, emitUpdate = true, parseOptions = {} } = {}) => ({ editor: editor2, tr, dispatch, commands }) => {
  const { doc: doc3 } = tr;
...
var toggleList = (listTypeOrName, itemTypeOrName, keepMarks, attributes = {}) => ({ editor: editor2, tr, state, dispatch, chain, commands, can }) => {
  ...
  for (let i = 0; i < plugins.length; i += 1) {
    ...
    if (plugin.spec.isInputRules && (undoable = plugin.getState(state))) {
      if (dispatch) {
        ...
        for (let j3 = toUndo.steps.length - 1; j3 >= 0; j3 -= 1) {
          tr.step(toUndo.steps[j3].invert(toUndo.docs[j3]));
        }
        if (undoable.text) {
...
var unsetAllMarks = (options = {}) => ({ tr, dispatch, editor: editor2 }) => {
...
function runInDecorationApplyScope(editor2, callback) {
  var _a, _b;
  depthByEditor.set(editor2, ((_a = depthByEditor.get(editor2)) != null ? _a : 0) + 1);
  try {
    return callback();
  } finally {
    const remaining = ((_b = depthByEditor.get(editor2)) != null ? _b : 1) - 1;
    if (remaining > 0) {
      depthByEditor.set(editor2, remaining);
    } else {
      depthByEditor.delete(editor2);
    }
  }
}
          ...
          runInDecorationApplyScope(editor2, () => {
            for (const { name, spec } of entries) {
              ...
              const shouldRecompute = shouldRecomputeDecoration(
                spec,
                { editor: editor2, tr, oldState, newState },
                forced
              );
              if (!shouldRecompute) {
...
function run2(config) {
  const { editor: editor2, state, from, to, rule, pasteEvent, dropEvent } = config;
  const { commands, chain, can } = new CommandManager({
    editor: editor2,
    state
  });
...
var ExtensionManager = class {
  ...
  createDecorationPlugin() {
    ...
    this.decorationManager = new DecorationManager({ editor: editor2, entries });
    ...
    for (let i = 0; i < tokens.length; i += 1) {
      ...
      if (token.type === "text") {
        ...
        if (!isClosing && openMatch && !/\/>$/.test(raw)) {
          ...
          let foundIndex = -1;
          const parts = [raw];
          for (let j3 = i + 1; j3 < tokens.length; j3 += 1) {
            const t = tokens[j3];
...
// app/extensions/rawHtmlText.ts
var RawHtmlText = Extension.create({
  name: "rawHtmlText",
...
var MARKER_RE = /^<!--\s*align:\s*(left|center|right)\s*-->$/;
function normalizeHardBreaks(markdown) {
...
function docToMarkdownWithAlignment(editor2) {
  const manager = editor2.markdown?.manager ?? editor2.markdown;
  if (!manager) {
    return editor2.getMarkdown();
  }
  const blocks = [];
  editor2.state.doc.forEach((node) => {
    const md3 = manager.renderNodeToMarkdown(node.toJSON());
    const align = node.attrs?.textAlign || "";
    if (align && align !== "left" && ALIGN_VALUES.includes(align)) {
...
function parseAlignment(markdown) {
...
function restoreMarkdownSyntax(markdown) {
  const lines = markdown.split("\n");
  let fence = false;
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
...
function applyAlignmentDirectives(editor2, directives) {
  if (!editor2) {
    return;
  }
  for (const { index, align } of directives) {
    if (index < 0 || index >= editor2.state.doc.childCount) {
      continue;
    }
    let pos = 0;
    for (let j3 = 0; j3 < index; j3++) {
      const child = editor2.state.doc.child(j3);
      if (!child) {
        break;
      }
      pos += child.nodeSize;
    ...
    try {
      editor2.chain().setTextSelection(pos + 1).setTextAlign(align).run();
    } catch {
    }
  }
}
...
// test_roundtrip.mts
var CONTENT = `## Introduction to Python Web Apps
...
function makeEditor() {
  return new Editor({
    ...
    extensions: [
      ...
      index_default2,
      index_default3,
      index_default4.configure({ levels: [1, 2, 3] }),
      index_default5.extend({ renderMarkdown: () => "<br>\n" }),
      Markdown.configure({ html: true, hardBreak: false, link: { markdownLinks: true } }),
      index_default6.configure({ types: ["heading", "paragraph"] }),
      RawHtmlText
    ],
    content: "<p>Hello</p>"
  });
}
function reload(editor2, md3) {
  const { clean, directives } = parseAlignment(md3);
  require("fs").writeFileSync("test_clean.txt", clean);
  editor2.commands.setContent(clean, { contentType: "markdown" });
  applyAlignmentDirectives(editor2, directives, editor2.getJSON());
}
...
function docHeadingsCentered(doc3) {
  let found2 = false;
  doc3.forEach((n) => {
    if (n.type.name === "heading" && n.attrs && n.attrs.textAlign === "center") found2 = true;
  });
  return found2;
}
var editor = makeEditor();
console.log("markdown mgr?", !!editor.markdown, "parse?", typeof (editor.markdown && editor.markdown.parse));
editor.commands.setContent(CONTENT, { contentType: "markdown" });
editor.commands.setTextSelection(1);
editor.commands.setTextAlign("center");
var hd = editor.state.doc.firstChild;
console.log("HEADING attrs:", JSON.stringify(hd && hd.attrs), "type:", hd && hd.type.name);
var md = docToMarkdownWithAlignment(editor);
...
var j1 = editor.getJSON();
var center1 = docHeadingsCentered(editor.state.doc);
var breaks1 = countBreaks(j1);
var startBreak1 = breaksAtStartOfAnyPara(j1);
console.log("RELOAD1: heading centered?", center1, "| total breaks?", breaks1, "| start-of-line break?", startBreak1);
var md2 = docToMarkdownWithAlignment(editor);
reload(editor, md2);
var j2 = editor.getJSON();
var center2 = docHeadingsCentered(editor.state.doc);
var breaks2 = countBreaks(j2);
var startBreak2 = breaksAtStartOfAnyPara(j2);
...
var mdR = docToMarkdownWithAlignment(editor);
reload(editor, mdR);
var jR = editor.getJSON();
